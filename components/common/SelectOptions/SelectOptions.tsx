import React, { useState, useMemo } from 'react';

import { OptionI } from '@/types/common';
import { useWindowClick } from '@/hooks/useWindowClick';

import cls from './SelectOptions.module.scss';

type Props = {
  options: OptionI[];
  onClick: (o: OptionI) => void;
  onOutsideClick?: () => void;
  outsideClass?: string;
};

export const SelectOptions: React.FC<Props> = ({
  options,
  onClick,
  onOutsideClick,
  outsideClass
}) => {
  const [search, setSearch] = useState('');

  const filteredOptions = useMemo(() => {
    return options.filter((o) => {
      const s = search.toLowerCase();
      const name = o.name.toLowerCase();
      const description = o.description ? o.description.toLowerCase() : '';

      if (name.includes(s) || (description && description.includes(s))) {
        return true;
      }

      return false;
    });
  }, [options, search]);

  useWindowClick((e) => {
    if (
      onOutsideClick &&
      !e.target.closest(`.${cls.root}`) &&
      (outsideClass ? !e.target.closest(`.${outsideClass}`) : true)
    ) {
      onOutsideClick();
    }
  });

  return (
    <div className={cls.root}>
      <div className={cls.input}>
        <svg
          width="16"
          height="16"
          viewBox="0 0 16 16"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            opacity="0.92"
            d="M6.50006 1C9.53765 1 12.0001 3.46256 12.0001 6.50029C12.0001 7.74868 11.5842 8.89993 10.8835 9.82304L14.7791 13.7233C15.0718 14.0164 15.0715 14.4913 14.7785 14.784C14.4854 15.0767 14.0105 15.0764 13.7178 14.7834L9.82266 10.8839C8.89959 11.5847 7.74839 12.0006 6.50006 12.0006C3.46246 12.0006 1 9.53802 1 6.50029C1 3.46256 3.46246 1 6.50006 1ZM6.50006 2.5C4.2909 2.5 2.5 4.29098 2.5 6.50029C2.5 8.70961 4.2909 10.5006 6.50006 10.5006C8.70921 10.5006 10.5001 8.70961 10.5001 6.50029C10.5001 4.29098 8.70921 2.5 6.50006 2.5Z"
            fill="#7A869A"
          />
        </svg>

        <input
          value={search}
          onChange={(e) => setSearch(e.currentTarget.value)}
          placeholder="Поиск"
        />
      </div>

      <ul>
        {filteredOptions.map((o) => (
          <li key={o.id}>
            <button type="button" onClick={() => onClick(o)}>
              <p className={cls.name}>{o.name}</p>
              <p className={cls.description}>{o.description}</p>
            </button>
          </li>
        ))}
      </ul>
    </div>
  );
};
